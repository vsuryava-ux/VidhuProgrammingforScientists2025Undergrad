...
def simpsons_map(all_maps: dict[str, dict[str, int]]) -> dict[str, float]:
    ...
    for key, value in all_maps.items(): 
        simpsons_map[key] = simpsons_index(value)
...
def beta_diversity_matrix (
    ...
    """
    samples = all_maps.keys()
    samples_sorted = sorted(samples)
    beta_matrix = initialize_square_matrix(len(samples_sorted))

    for i in range(0, len(samples_sorted)): 
        for j in range(i, len(samples_sorted)): 
            if dist_metric == "Bray-Curtis":
                beta_matrix[i][j] =  bray_curtis_distance(all_maps[samples_sorted[i]], all_maps[samples_sorted[j]])
                beta_matrix[j][i] = beta_matrix[i][j]
            elif dist_metric == "Jaccard":
                beta_matrix[i][j] =  jaccard_distance(all_maps[samples_sorted[i]], all_maps[samples_sorted[j]])
                beta_matrix[j][i] = beta_matrix[i][j]
            else: 
                print("bad this isnt a good distance metric")

    return (samples_sorted, beta_matrix)
